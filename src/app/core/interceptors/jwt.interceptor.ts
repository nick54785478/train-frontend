  ...
  return authService.getJwtToken().pipe(
    ...
    switchMap((token) => {
      ...
      if (authService.checkExpired(token)) {
        console.log('過期');
        setTimeout(() => {
          storageService.removeLocalStorageItem(SystemStorageKey.JWT_TOKEN); // 清除過期 Token
          storageService.removeSessionStorageItem(SystemStorageKey.JWT_TOKEN);
      ...
      return next(req).pipe(
        catchError((error: HttpErrorResponse) => {
          ...
          if (error.status === 500) {
            ...
            setTimeout(() => {
              storageService.removeLocalStorageItem(SystemStorageKey.JWT_TOKEN); // 清除過期 Token
              storageService.removeSessionStorageItem(
                SystemStorageKey.JWT_TOKEN
              );
              router.navigate(['/login']); // 導向登入頁
            }, 5000);
